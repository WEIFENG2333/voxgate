...
func WriteResult(w io.Writer, format string, result asr.Result) error {
	...
	case NDJSON:
		return json.NewEncoder(w).Encode(asr.Event{Type: asr.EventTranscriptCompleted, Text: result.Text, Duration: result.Duration})
	default:
...
func WriteEvent(w io.Writer, format string, event asr.Event) error {
	...
	case Text:
		if event.Type == asr.EventTranscriptDelta || event.Type == asr.EventTranscriptCompleted {
